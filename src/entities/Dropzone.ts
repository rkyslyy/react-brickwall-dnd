...
export class Dropzone {
  ...
  repositionItems = (
    ...
    let xOffset = gridGap;
    let yOffset = gridGap;
    let resultingContainerHeight =
      parseInt(this.container.style.minHeight) || DEFAULT_MIN_HEIGHT;
    let lowestItemLevel = 0;
    ...
    this.items.forEach((item) => {
      ...
      if (xOffset + item.getFullWidth() + gridGap > maxWidth) {
        xOffset = gridGap;
        yOffset = lowestItemLevel + gridGap;
      }
      ...
      if (yOffset + item.rect.height > lowestItemLevel) {
        lowestItemLevel = yOffset + item.rect.height;
      }
